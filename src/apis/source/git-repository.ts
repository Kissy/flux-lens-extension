...
export class GitRepository extends Renderer.K8sApi.KubeObject<KubeObjectMetadata, GitRepositoryStatus, GitRepositorySpec> {
    ...
    getSecretDetailUrl(): string {
        const secretId = { namespace: this.metadata.namespace, name: this.spec.secretRef.name }
        return Renderer.Component.getDetailsUrl(Renderer.K8sApi.secretsApi.getUrl(secretId))
    }
}
