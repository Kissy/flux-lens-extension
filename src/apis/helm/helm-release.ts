...
export class HelmRelease extends Renderer.K8sApi.KubeObject<KubeObjectMetadata, HelmReleaseStatus, HelmReleaseSpec> {
    ...
    getSourceDetailUrl(): string {
        let helmChartComponent = this.status.helmChart.split("/")
        const helmChartId = { namespace: helmChartComponent[0], name: helmChartComponent[1] }
