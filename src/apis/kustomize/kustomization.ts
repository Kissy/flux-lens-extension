...
import {KubeObject, KubeObjectMetadata} from "@k8slens/extensions/dist/src/common/k8s-api/kube-object";
...
export class Kustomization extends Renderer.K8sApi.KubeObject<KubeObjectMetadata, KustomizationStatus, KustomizationSpec> {
    ...
    getStatusMessage(): string {
        ...
        const ready = this.status.conditions
            .find(({ type }) => type === "Ready");
        switch (ready.reason) {
    ...
    getSourceDetailUrl(): string {
        ...
        let matchingApi = Renderer.K8sApi.apiManager.getApi((api: KubeApi<KubeObject>) => {
            let matchingVersion = !sourceRef.apiVersion || api.apiVersion == sourceRef.apiVersion;
            return api.apiGroup == "source.toolkit.fluxcd.io" && matchingVersion && api.kind == sourceRef.kind
        });
        let namespace = sourceRef.namespace ? sourceRef.namespace : this.metadata.namespace
