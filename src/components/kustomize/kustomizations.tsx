...
export class Kustomizations extends React.Component<{ extension: Renderer.LensExtension }, any> {

    render() {
        return (
            <Renderer.Component.KubeObjectListLayout
                className="FluxKustomizations" store={kustomizationsStore}
                ...
                renderTableHeader={[
                    ...
                    {title: "Source", className: "source", sortBy: columnId.source, id: columnId.source},
                    {title: "Path", className: "path", sortBy: columnId.path, id: columnId.path},
                ...
                renderTableContents={(kustomization: Kustomization) => [
                    ...
                    <Link to={kustomization.getSourceDetailUrl()} onClick={Common.Util.stopPropagation}>{kustomization.spec.sourceRef.name}</Link>,
                    kustomization.spec.path,
                    kustomization.spec.interval,
                    kustomization.getAge(),
                    { title: kustomization.getStatusMessage(), className: kustomization.getStatusClassName() }
                ]}
            />
        )
    }
