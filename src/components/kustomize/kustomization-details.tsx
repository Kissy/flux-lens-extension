...
export interface FluxKustomizationDetailsProps extends Renderer.Component.KubeObjectDetailsProps<Kustomization> {
}

export class KustomizationDetails extends React.Component<FluxKustomizationDetailsProps, any> {
    render() {
        ...
        return (
            <div className="FluxKustomization">
                ...
                <Renderer.Component.DrawerItem name="Status">
                    <span className={Common.Util.cssNames("status", fluxKustomization.getStatusClassName())}>{fluxKustomization.getStatusMessage()}</span>
                </Renderer.Component.DrawerItem>
                {fluxKustomization.spec.dependsOn &&
                <Renderer.Component.DrawerItem name="Depends On" className="depends-on" labelsOnly>
                    {
                        fluxKustomization.spec.dependsOn.map(dependency => {
                            const { namespace, name } = dependency;
                            return (
                                <Renderer.Component.Badge key={namespace + "/" + name} label={namespace + "/" + name} />
                            );
                        })
                    }
                </Renderer.Component.DrawerItem>
