...
export interface FluxHelmChartDetailsProps
  extends Renderer.Component.KubeObjectDetailsProps<HelmChart> {}

export class HelmChartDetails extends React.Component<
    ...
    return (
      <div className="FluxHelmChart">
        ...
        <Renderer.Component.DrawerItem name="Status">
          <span
            className={Common.Util.cssNames(
              "status",
              kebabCase(fluxHelmChart.getStatusMessage())
            )}
          >
            {fluxHelmChart.getStatusMessage()}
          </span>
        </Renderer.Component.DrawerItem>
        <Renderer.Component.DrawerItem name="Reconciled At">
          {fluxHelmChart.getReconciledAge()} ago (
          {fluxHelmChart.status.artifact?.lastUpdateTime})
        </Renderer.Component.DrawerItem>
        ...
        <Renderer.Component.DrawerItem name="Repository">
          <Link to={fluxHelmChart.getSourceDetailUrl()}>
            {fluxHelmChart.spec.sourceRef.name}
          </Link>
        </Renderer.Component.DrawerItem>
        ...
        {fluxHelmChart.spec.valuesFiles && (
          <Renderer.Component.DrawerItem name="Values Files">
            {fluxHelmChart.spec.valuesFiles.map(file => {
              return <Renderer.Component.Badge key={file} label={file} />;
            })}
          </Renderer.Component.DrawerItem>
        )}
        {fluxHelmChart.spec.valuesFile && (
          <Renderer.Component.DrawerItem name="Values File">
            <Renderer.Component.Badge
              key={fluxHelmChart.spec.valuesFile}
              label={fluxHelmChart.spec.valuesFile}
            />
          </Renderer.Component.DrawerItem>
        )}
      </div>
