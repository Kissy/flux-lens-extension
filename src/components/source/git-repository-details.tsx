...
import {LocaleDate} from "@k8slens/extensions/dist/src/renderer/components/locale-date";
import {ExternalLink} from "../external-link";
import {Link} from "react-router-dom";

export interface FluxGitRepositoryDetailsProps extends Renderer.Component.KubeObjectDetailsProps<GitRepository> {
}

export class GitRepositoryDetails extends React.Component<FluxGitRepositoryDetailsProps, any> {
    render() {
        ...
        return (
            <div className="FluxGitRepository">
                ...
                <Renderer.Component.DrawerItem name="Status">
                    <span className={Common.Util.cssNames("status", kebabCase(fluxGitRepository.getStatusMessage()))}>{fluxGitRepository.getStatusMessage()}</span>
                </Renderer.Component.DrawerItem>
                <Renderer.Component.DrawerItem name="Reconciled At">
                    {fluxGitRepository.getReconciledAge()} ago ({fluxGitRepository.status.artifact?.lastUpdateTime})
                </Renderer.Component.DrawerItem>
                ...
                <Renderer.Component.DrawerItem name="Secret">
                    <Link to={fluxGitRepository.getSecretDetailUrl()}>{fluxGitRepository.spec.secretRef.name}</Link>
                </Renderer.Component.DrawerItem>
                }
                ...
                <React.Fragment>
                    <Renderer.Component.DrawerItem name="Branch" hidden={!fluxGitRepository.spec.ref.branch}>
                        {fluxGitRepository.spec.ref.branch}
                    </Renderer.Component.DrawerItem>
                    <Renderer.Component.DrawerItem name="Tag" hidden={!fluxGitRepository.spec.ref.tag}>
                        {fluxGitRepository.spec.ref.tag}
                    </Renderer.Component.DrawerItem>
                    <Renderer.Component.DrawerItem name="Semver" hidden={!fluxGitRepository.spec.ref.semver}>
                        {fluxGitRepository.spec.ref.semver}
                    </Renderer.Component.DrawerItem>
                    <Renderer.Component.DrawerItem name="Commit" hidden={!fluxGitRepository.spec.ref.commit}>
                        {fluxGitRepository.spec.ref.commit}
                    </Renderer.Component.DrawerItem>
                </React.Fragment>
                }
                {fluxGitRepository.spec.verify &&
                <Renderer.Component.DrawerItem name="Verify" hidden={!fluxGitRepository.spec.verify.mode}>
                    {fluxGitRepository.spec.verify.mode}
                </Renderer.Component.DrawerItem>
                }
                {fluxGitRepository.spec.ignore &&
                <Renderer.Component.DrawerItem name="Ignore">
                    <Renderer.Component.Input multiLine theme="round-black" className="box grow"
