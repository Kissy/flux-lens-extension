...
export class HelmCharts extends React.Component<{ extension: Renderer.LensExtension }, any> {

    render() {
        return (
            <Renderer.Component.KubeObjectListLayout
                className="FluxHelmCharts" store={helmChartsStore}
                ...
                renderTableHeader={[
                    ...
                    {title: "Repository", className: "repository", sortBy: columnId.repository, id: columnId.repository},
                    {title: "Interval", className: "interval", sortBy: columnId.interval, id: columnId.interval},
                    {title: "Age", className: "age", sortBy: columnId.age, id: columnId.age},
                    {title: "Installed version ", className: "revision", sortBy: columnId.revision, id: columnId.revision},
                ...
                renderTableContents={(helmChart: HelmChart) => [
                    ...
                    <Link to={helmChart.getSourceDetailUrl()} onClick={Common.Util.stopPropagation}>{helmChart.spec.sourceRef.name}</Link>,
                    helmChart.spec.interval,
                    helmChart.getAge(),
                    helmChart.getRevision(),
                    helmChart.getReconciledAge(),
                    { title: helmChart.getStatusMessage(), className: kebabCase(helmChart.getStatusMessage()) }
                ]}
            />
        )
    }
