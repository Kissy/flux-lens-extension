...
export class HelmReleases extends React.Component<
  ...
  render(): ReactNode {
    return (
      <Renderer.Component.KubeObjectListLayout
        className="FluxHelmReleases"
        store={helmReleasesStore}
        ...
        renderTableContents={(helmRelease: HelmRelease) => [
          ...
          <Link
            to={helmRelease.getSourceDetailUrl()}
            onClick={Common.Util.stopPropagation}
          >
            {helmRelease.spec.chart.spec.chart}
          </Link>,
          helmRelease.spec.interval,
          helmRelease.getAge(),
          {
            title: helmRelease.getStatusMessage(),
            className: helmRelease.getStatusClassName()
          }
        ]}
      />
    );
